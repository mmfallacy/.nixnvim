...
Before proposing a sequence, check [partial-staging feasibility](./partial-staging.md) for every shared file. A tracked file can be split with a small, reviewed patch based on its raw Git diff; an untracked file can be split with a small new-file patch based on reading its contents. In both cases, each staged slice and remainder must be coherent and the patch constructible under the allowed commands. If not, stop and request an editing agent to split the file before offering a sequence; do not change the index as a workaround or bundle the behaviors.
...
**Scenario — two consumers of entry cardinality:** Declaration parsing rejects invalid input; assembly guards against state mutated after parsing. Their direct tests share `tests/declarations.test.ts`. If tracked, inspect the raw diff for a reviewable declaration patch; if untracked, read the file and check whether a small new-file patch can produce a valid declaration test slice, leaving the assembly tests for later. Propose separate groups only when that split is feasible; otherwise request a file split. An unrelated sourced-file mode assertion in `src/build/files.ts` needs its own explanation, exclusion, or decision; its path alone does not place it in the assembly group.
